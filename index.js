  ...
  return new Promise((resolve, reject) => {
    request(option, (error, response) => {
      if (error) {
        ...
        reject(error);
      } else if (response.statusCode !== 500 && response.body) {
        try {
        ...
        typeof _data === "string" &&
          _data.replaceAll(/https:\/\/github\.com/g, "/");
        resolve({
          body: _data,
        ...
        resolve({
          statusCode: 500,
          body: '<h1 align="center" style="color: red;">Server error</h1>',
          headers: {},
        });
      }
    });
...
app.use(async (ctx) => {
  ...
  if (!url || !isUrl) {
    if (!piperaOrigin) {
      ...
      console.log("SETTING originURL.origin", originURL.origin);
